...
{
    public partial class TimMinNhanSu : Form
    {
        public TimMinNhanSu()
        {
            InitializeComponent();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            float min = float.Parse((string)result1.Rows[0]["DIEM"]);
            int i = 1;
            while (i < a)
            {
                if(float.Parse((string)result1.Rows[i]["DIEM"])<min)
                {
                    min = float.Parse((string)result1.Rows[i]["DIEM"]);
                }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            float min = float.Parse((string)result1.Rows[0]["DIEMDANHLAB"]);
            int i = 1;
            while (i < a)
            {
                if (float.Parse((string)result1.Rows[i]["DIEMDANHLAB"]) < min)
                {
                    min = float.Parse((string)result1.Rows[i]["DIEMDANHLAB"]);
                }
